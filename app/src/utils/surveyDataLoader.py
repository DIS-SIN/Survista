...
class SurveyDataLoader:
    def __init__(self, path_to_raw_folder,
        ...
        if os.path.isdir(self.path_to_raw_folder) == False and not survey_preprocessed:
            error = RawdataFolderNotExist(
                'The path_to_raw_folder {path} is not valid'.format(path=self.path_to_raw_folder))
            self.__error_logger(error)
        self.path_to_metadata = os.path.join(
            self.path_to_raw_folder, 'survey_metadata.csv')
        # attempt loading the survey metadata and then check it has a valid format
        try:
            self.metadata = pd.read_csv(
                self.path_to_metadata, encoding='utf-8', index_col=0)
        ...
        if survey_preprocessed:
            self.path_to_index = os.path.join(
                self.path_to_processed_folder, 'question_index.csv')
        else:
            self.path_to_index = os.path.join(
                self.path_to_raw_folder, 'question_index.csv')
        if not os.path.isfile(self.path_to_index):
            if survey_preprocessed:
                self.path_to_index_raw = os.path.join(
                    self.path_to_raw_folder, 'question_index.csv')
                if not os.path.isfile(self.path_to_index_raw):
                    error = QuestionIndexFileNotExist(
                        'Both raw folder and processed folder do not contain question_index.csv')
                    self.__error_logger(error)
                else:
                    shutil.copy2(self.path_to_index_raw, self.path_to_index)
            else:
                error = QuestionIndexFileNotExist(
                    'Folder does not contain question_index.csv')
                self.__error_logger(error)
        # check if question index file is of right format
        try:
            self.index_df = pd.read_csv(
                self.path_to_index, encoding='utf-8', index_col=0)
        ...
        if os.path.isdir(self.path_to_processed_folder) and not survey_preprocessed:
            ...
            copy_tree(self.path_to_raw_folder, self.path_to_processed_folder)
        self.path_to_comments_folder = os.path.join(
            self.path_to_processed_folder, 'comments')
    ...
    def __init_dict(self):
        ...
        if self.metadata_columns['Number_Of_Questions'] != False:
            results_dict['total_number_of_questions'] = int(
                self.metadata_columns['Number_Of_Questions'])
        ...
        for i in self.index_df.index:
            try:
                ...
                question_file = os.path.join(
                    self.path_to_raw_folder, 'q' + i + '.csv')
                ...
                raise FileExistsError(
                    'Question file {q_file} could not be found, please modify your index or restore the file'.format(q_file=question_file))
                ...
                if not question is None:
                    ...
                    try:
                        file_path = os.path.join(
                            self.path_to_raw_folder, filename)
                        ...
                        raise FileNotFoundError(
                            'failed to load {path}'.format(file_path))
    ...
    def __load_sentiment(self):
        ...
        for question in questions:
            ...
            if current_section['has_comments'] == True:
                ...
                section = CommentSectionLoader(
                    current_section, question, self.path_to_comments_folder)
    ...
    def __to_json(self):
        ...
        json_obj['number_of_comment_sections'] = int(
            json_obj['number_of_comment_sections'])
    ...
    def __get_key(self):
        ...
        if key is None:
            ...
            key = db.execute("SELECT id FROM index_table WHERE raw_data_path = '{path}'".format(
                path=self.path_to_raw_folder)).fetchone()['id']
    ...
    def __update_survey_entry_details(self):
        db = self.db
        self.path_to_processed_folder = os.path.join(
            self.path_to_processed_folder, str(self.key))
        self.path_to_json_file = os.path.join(
            self.path_to_json_folder, str(self.key) + '.json')
    ...
    def __get_status(self, status_id):
        db = self.db
        status = db.execute(("SELECT status_name FROM statuses WHERE id = {id} ;".format(
            id=status_id))).fetchone()
    ...
    def __get_survey_entry(self):
        ...
        res = db.execute(
            'SELECT * FROM index_table WHERE id = {id}'.format(id=self.key)).fetchone()
    ...
    def __completed(self):
        ...
        UPDATE index_table
         ...
         WHERE id = {my_id} ;
        """.format(status="(SELECT id FROM statuses WHERE status_name = 'completed')", my_id=self.key).replace("\n", "")))
        db.execute(("""
                INSERT INTO logs(index_id, log_type_id, log_text, date_of_log)
                 VALUES({id},{t},'{l}','{d}');""".format(id=self.key,
    ...
    def __error_logger(self, error):
        ...
        query = "SELECT id FROM errors WHERE error = '{error_name}'".format(
        ...
        for column in self.metadata.columns:
            ...
            if metacol is None:
                ...
                if column_value == "" or column_value != column_value:
                    error = MetadataFormatNotValid(
                        "Columns included in metadata csv must not be empty")
                    self.__error_logger(error)
                else:
                    metadata_columns[column] = column_value
        if metadata_columns["Title_Of_Survey"] == False:
            error = MetadataFormatNotValid(
                "Column Title_Of_Survey in metadata.csv must be provided")
            self.__error_logger(error)
        if metadata_columns["Date_Of_Survey"] == False:
            error = MetadataFormatNotValid(
                "Column Date_Of_Survey in metadata.csv must be provided")
            self.__error_logger(error)
    ...
    def __check_index_format(self):
        ...
        for column in self.index_df.columns:
            ...
            if index_col is None:
                error = QuestionIndexFormatNotValid(
                    "Column " + column + " is not valid")
                self.__error_logger(error)
            else:
...
class CommentSectionLoader:
    def __init__(self, sectionDict,
        ...
        self.path_to_section = os.path.join(
            path_to_comments, "section_" + section_key)
...
class CommentLoader:
    def __init__(self, comment_key, section_key, comment, path_to_section):
        ...
        self.path_to_comment = os.path.join(
            path_to_section, "comment_" + comment_key)
        ...
        if os.path.isdir(self.path_to_comment) == False:
            ...
            self.sentiment_score = comment_df.loc[comment_key,
                                                  'sentiment_score']
            self.magnitude_score = comment_df.loc[comment_key,
                                                  'magnitude_score']
            self.number_of_sentences = int(
                comment_df.loc[comment_key, 'number_of_sentences'])
            self.number_of_words = int(
                comment_df.loc[comment_key, 'number_of_words'])
            for index in range(0, self.number_of_sentences):
                sentence_key = comment_key + '-' + str(index)
                current_sentence = Sentence(
                    sentence_key, comment_key, self.path_to_comment, self.language)
                self.sentences_frames.append(
                    current_sentence.get_sentences_frame())
    ...
    def __init_sentiment(self):
        ...
        document = types.Document(
            content=self.comment, type=enums.Document.Type.PLAIN_TEXT)
        ...
        for sentence in sentiment.sentences:
            ...
            current_sentence = Sentence(
                sentence_key, self.comment_key, self.path_to_comment, self.language, sentence)
        ...
        comment_df.loc[self.comment_key,
        ...
        self.comment_frame = comment_df
        comment_df.to_csv(os.path.join(
            self.path_to_comment, 'comment_' + self.comment_key + '.csv'), encoding='utf-8')
...
class Sentence:
    def __init__(self, sentence_key, comment_key, path_to_comment, comment_language=None, sentence=None):
        self.path_to_sentence = os.path.join(
            path_to_comment, "sentence_" + sentence_key + ".csv")
        if os.path.isfile(self.path_to_sentence) == False:
            ...
            self.sentiment_score = sentence_df.loc[sentence_key,
                                                   'sentiment_score']
            self.magnitude_score = sentence_df.loc[sentence_key,
                                                   'magnitude_score']
            self.number_of_words = int(
                sentence_df.loc[sentence_key, 'number_of_words'])
    ...
    def __calculate_number_of_words(self):
        ...
        punctuation = [",", ".", "?", ";", "!", ":",
                       ">", "<", "{", "}", "[", "]", "\\", "/"]
        def no_punct(arr, punct): return [y[0]
                                          for y in zip(arr) if not y[0] in punct]
